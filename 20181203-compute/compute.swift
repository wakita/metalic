...
func main() {
  ...
  do { try library = device.makeLibrary(filepath: "compute.metallib")
  } catch { print(error.localizedDescription) }
  guard let add = library.makeFunction(name: "add") else { return }
  var pipelineState: MTLComputePipelineState!
  do { pipelineState = try device.makeComputePipelineState(function: add)
  } catch { print(error.localizedDescription) }
